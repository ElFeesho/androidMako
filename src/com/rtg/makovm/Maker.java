...
public class Maker implements MakoConstants
{
	...
	private final Map<String, Integer> dictionary = new TreeMap<String, Integer>();
	private final Map<String, Integer> variables = new TreeMap<String, Integer>();
	private final Map<String, List<Integer>> prototypes = new HashMap<String, List<Integer>>();
	private final Map<String, List<Integer>> references = new HashMap<String, List<Integer>>();
	private final MakoRom rom;
	private boolean compiling = false;
	private String wordName = null;
	private final Stack<Integer> branchStack = new Stack<Integer>();
	private final Stack<Integer> loopStack = new Stack<Integer>();
	private final Queue<Integer> breaks = new LinkedList<Integer>();
	...
	public static boolean compile(String inputFile, String output)
	{
		...
		compiler.rom.optimize = true;

		try
		{
			compiler.compileToken(":include", tokens("<Lang.fs>"));
		}
		catch (IOException f)
		{
	...
	public Maker(LibraryProvider aLibraryProvider)
	{
		this(new MakoRom());

		mLibraryProvider = aLibraryProvider;
		...
		/*
		 * defineVariable("XO", XO); defineVariable("XA", XA);
	...
	private void compileFile(String filename) throws IOException
	{
		...
		/*
		 * System.out.println("Compiling: " + new File( currentPath.peek(), new
		 * File(filename).getName() ));
		 */

		FileReader in = new FileReader(new File(currentPath.peek(), new File(filename).getName()));
	...
	private void compileToken(String token, Queue<Object> tokens) throws IOException
	{
		...
		else if (token.equals(":array"))
		{
			...
			{
				rom.add(value, MakoRom.Type.Array);
			}
		}
		...
		else if (token.equals(":table"))
		{
			...
			{
				rom.add(i, MakoRom.Type.Data); // CHANGE TO ARRAY TYPE AFTER
												// TESTING!!!!!!
			}
		...
		else if (token.equals(":image"))
		{
			...
			String fileName = new File(currentPath.peek(), new File(unquote(tokens.remove().toString())).getName()).toString();
		...
		else if (token.equals(":include"))
		{
			...
			{
				String libPath = mLibraryProvider.getLibraryDirectory().getAbsolutePath();
		...
		else if (token.equals("2dup"))
		{
			rom.addOver();
			rom.addOver();
		}
		else if (token.equals("2drop"))
		{
			rom.addDrop();
			rom.addDrop();
		}
		else if (token.equals("rdrop"))
		{
			rom.addRts();
			rom.addDrop();
		}
		else if (token.equals("nip"))
		{
			rom.addSwap();
			rom.addDrop();
		}
		...
		else if (variables.containsKey(token))
		{
			...
			{
				rom.add(address, MakoRom.Type.Array);
			}
		}
	...
	private void clearDef(String name)
	{
		if (guardShadow)
		{
			if (variables.containsKey(name))
			{
				System.out.format("shadowed value '%s'%n", name);
			}
			if (dictionary.containsKey(name))
			{
				System.out.format("shadowed word '%s'%n", name);
			}
		}
	...
	private int getConstant(Object o)
	{
		...
		if (variables.containsKey(o.toString()))
		{
			return variables.get(o.toString());
		}
		if (dictionary.containsKey(o.toString()))
		{
			return dictionary.get(o.toString());
		}
	...
	public void disassemble(String word)
	{
		...
		int last = first + 1;
		while (last < rom.size() && "".equals(rom.getLabel(last)))
		{
			last++;
		}
	...
	private static Queue<Object> tokens(String text)
	{
		...
		while (index < text.length())
		{
			...
			// Forth-style paren comments (so I can have normal notation for
			// stack diagrams)
